...
public class ImageFilter {
	...
	public File getImageFile() {
		return ImageIOHelper.createPngImage(image);
	}
	...
	public ImageFilter(File image) {
		...
		this.image = ImageIOHelper.getImage(image);
	...
	public ImageFilter sharp() {
		...
		this.image = ImageIOHelper.imageProducerToBufferedImage(new MemoryImageSource(iw, ih, tempPixels, 0, iw));
	...
	public ImageFilter scaling(double s) {
	...
	public ImageFilter scale(Float s) {
	...
	public ImageFilter filterColor(int muys,int dis) {
		...
		for (int y = 1; y < ih - 1; y++) {
			for (int x = 1; x < iw - 1; x++) {
				if(pixels[x + y * iw]==0xffffffff){
					continue;
				}
				if (checkColor(pixels, x, y,muys,dis) ) {
					pixels[x + y * iw] = 0xff000000;
				} else {
					pixels[x + y * iw] = 0xffffffff;
				}
	...
	private boolean checkColor(int[] cs, int x, int y,int muys,int dis) {
//		if (x < 0 || x >= iw || y < 0 || y >= ih) {
//			return false;
//		}
		if(colorDis(muys,cs[x + y * iw])<dis){
			return true;
		}else{
			return false;
		}
	...
	int colorDis(int s,int d){
		return Math.abs(((s&0x00ff0000)>>16)-((d&0x00ff0000)>>16))+Math.abs(((s&0x0000ff00)>>8)-((d&0x0000ff00)>>8))+Math.abs(((s&0x000000ff))-((d&0x000000ff)));
	}
